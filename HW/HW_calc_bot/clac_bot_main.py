...
bot = telebot.TeleBot('6167659311:AAFaW5h-VBax50oloWjnQ-QWs59tGZRCJ6k')


def log(message):
    file = open('data.csv', 'a')
    file.write(
        f'{message.from_user.first_name}, {datetime.datetime.now()}, {message.text}\n')
    file.close()
...
def controller(message):
    ...
    if message.text == 'Целые':
        ...
        bot.send_message(
            message.chat.id, 'Введите свой пример, расставляя пробел между знаками')
